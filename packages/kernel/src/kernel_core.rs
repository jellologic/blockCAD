...
use crate::feature_tree::evaluator::{evaluate, evaluate_with_metrics, EvalMetrics};
...
impl KernelCore {
    ...
    /// Evaluate and return metrics about cache effectiveness.
    pub fn evaluate_with_metrics(&mut self) -> KernelResult<EvalMetrics> {
        let (_brep, metrics) = evaluate_with_metrics(&mut self.tree)?;
        Ok(metrics)
    }
